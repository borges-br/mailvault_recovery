...
- O core (`MailVault.Core`) define os contratos de abstração e os pipelines forenses sem referenciar tecnologias específicas.
- O executável da CLI (`MailVault.Cli`) referncia apenas o Domain, Core e Audit, mas **não** pode referenciar diretamente os adapters de arquivos `.ost/.pst` em tempo de compilação.
...
1. **Testabilidade Absoluta**: Nos testes de unidade e testes integrados de CLI da Milestone 3, injetamos dinamicamente um `FakeMailStoreReader.cs` no Program, permitindo exercitar o pipeline da CLI sem precisar carregar arquivos PST reais ou usar componentes de terceiros instáveis.
